...
class RepossessListTerms_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'repo_list_terms_';
		$this->from = 'repossess_list_terms';
		$this->id = 'repo_list_terms_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
		$this->orderBy = 'sort';
		$this->orderType = 'asc';
	...
	public function deleteDataList($data){
		...
		if( isset($data['repo_list_chapter_id']) && $data['repo_list_chapter_id'] !=''){
			$data['repo_list_chapter_id'] = $this->db->escape_str($data['repo_list_chapter_id']);
			$this->db->where('repo_list_chapter_id', $data['repo_list_chapter_id'], FALSE);
		}
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			...
			$this->db->where('repo_list_terms_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('repo_list_terms_created_date <=', $queryData['endDate'], FALSE);
		}

		if( isset($queryData['repo_list_chapter_id']) && $queryData['repo_list_chapter_id'] !=''){
			$queryData['repo_list_chapter_id'] = $this->db->escape($queryData['repo_list_chapter_id']);
			$this->db->where('repo_list_chapter_id', $queryData['repo_list_chapter_id'], FALSE);
		}
