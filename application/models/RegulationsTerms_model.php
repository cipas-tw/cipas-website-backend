...
class RegulationsTerms_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'law_terms_';
		$this->from = 'regulations_terms';
		$this->id = 'law_terms_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
		$this->orderBy = 'sort';
		$this->orderType = 'asc';
	...
	public function deleteDataList($data){
		...
		if( isset($data['law_chapter_id']) && $data['law_chapter_id'] !=''){
			$data['law_chapter_id'] = $this->db->escape_str($data['law_chapter_id']);
			$this->db->where('law_chapter_id', $data['law_chapter_id'], FALSE);
		}
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['law_chapter_id']) && $queryData['law_chapter_id'] !=''){
			$queryData['law_chapter_id'] = $this->db->escape_str($queryData['law_chapter_id']);
			$this->db->where('law_chapter_id', $queryData['law_chapter_id'], FALSE);
		}

		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			$queryData['startDate'] = $this->db->escape($queryData['startDate'].' 00:00:00');
			$this->db->where('law_terms_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('law_terms_created_date <=', $queryData['endDate'], FALSE);
		}
