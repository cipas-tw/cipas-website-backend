...
class PurchasesFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'purchase_file_';
		$this->from = 'purchases_file';
		$this->id = 'purchase_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['purchase_id']) && $data['purchase_id'] !=''){
			$data['purchase_id'] = $this->db->escape_str($data['purchase_id']);
			$this->db->where('purchase_id', $data['purchase_id'], FALSE);
		}
