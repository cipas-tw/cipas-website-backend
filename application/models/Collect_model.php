...
class Collect_model extends MY_BackendModel {
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			...
			$this->db->where('collect_plan_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('collect_plan_created_date <=', $queryData['endDate'], FALSE);
		}
	...
	public function getcollect_planTypeList(){
		$this->db->from('collect_plan_type')
				 ->where('collect_plan_type_is_del',0,false)
				 ->order_by('collect_plan_type_created_date', 'asc');
