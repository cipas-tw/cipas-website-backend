...
class StatisticsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'sales_file_';
		$this->from = 'statistics_file';
		$this->id = 'sales_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['sales_id']) && $data['sales_id'] !=''){
			$data['sales_id'] = $this->db->escape_str($data['sales_id']);
			$this->db->where('sales_id', $data['sales_id'], FALSE);
		}
