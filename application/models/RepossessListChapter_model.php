...
class RepossessListChapter_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'repo_list_chapter_';
		$this->from = 'repossess_list_chapter';
		$this->id = 'repo_list_chapter_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			...
			$this->db->where('repo_list_chapter_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('repo_list_chapter_created_date <=', $queryData['endDate'], FALSE);
		}
