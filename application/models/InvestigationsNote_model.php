...
class InvestigationsNote_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'survey_note_';
		$this->from = 'investigations_note';
		$this->id = 'survey_note_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['survey_id']) && $data['survey_id'] !=''){
			$data['survey_id'] = $this->db->escape_str($data['survey_id']);
			$this->db->where('survey_id', $data['survey_id'], FALSE);
		}
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['survey_id']) && $queryData['survey_id'] !=''){
			$queryData['survey_id'] = $this->db->escape($queryData['survey_id']);
			$this->db->where('survey_id', $queryData['survey_id'], FALSE);
		}
