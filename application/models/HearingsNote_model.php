...
class hearingsNote_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'hearing_note_';
		$this->from = 'hearings_note';
		$this->id = 'hearing_note_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['hearing_id']) && $data['hearing_id'] !=''){
			$data['hearing_id'] = $this->db->escape_str($data['hearing_id']);
			$this->db->where('hearing_id', $data['hearing_id'], FALSE);
		}
