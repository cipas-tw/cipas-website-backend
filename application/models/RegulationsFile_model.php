...
class RegulationsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'law_file_';
		$this->from = 'regulations_file';
		$this->id = 'law_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['law_id']) && $data['law_id'] !=''){
			$data['law_id'] = $this->db->escape($data['law_id']);
			$this->db->where('law_id', $data['law_id'], FALSE);
		}
