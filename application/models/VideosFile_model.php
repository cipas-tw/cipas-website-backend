...
class VideosFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'video_file_';
		$this->from = 'videos_file';
		$this->id = 'video_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['video_id']) && $data['video_id'] !=''){
			$data['video_id'] = $this->db->escape_str($data['video_id']);
			$this->db->where('video_id', $data['video_id'], FALSE);
		}
