...
class RecoveriesFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'repo_file_';
		$this->from = 'recoveries_file';
		$this->id = 'repo_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['repo_id']) && $data['repo_id'] !=''){
			$data['repo_id'] = $this->db->escape_str($data['repo_id']);
			$this->db->where('repo_id', $data['repo_id'], FALSE);
		}
