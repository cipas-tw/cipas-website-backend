...
class AlbumsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'photo_file_';
		$this->from = 'albums_file';
		$this->id = 'photo_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
		$this->orderBy = 'sort';
		$this->orderType = 'ASC';
	...
	public function deleteDataList($data){
		...
		if( isset($data['photo_id']) && $data['photo_id'] !=''){
			$data['photo_id'] = $this->db->escape_str($data['photo_id']);
			$this->db->where('photo_id', $data['photo_id'], FALSE);
		}
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['photo_id']) && $queryData['photo_id'] !=''){
			$queryData['photo_id'] = is_array($queryData['photo_id'])? $queryData['photo_id'] : [$queryData['photo_id']];
			$this->db->where_in('photo_id', $queryData['photo_id'], FALSE);
		}
