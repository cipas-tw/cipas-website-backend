...
class ArticlesFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'news_file_';
		$this->from = 'articles_file';
		$this->id = 'news_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['news_id']) && $data['news_id'] !=''){
			$data['news_id'] = $this->db->escape_str($data['news_id']);
			$this->db->where('news_id', $data['news_id'], FALSE);
		}
