...
class JournalsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'journal_file_';
		$this->from = 'journals_file';
		$this->id = 'journal_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['journal_id']) && $data['journal_id'] !=''){
			$data['journal_id'] = $this->db->escape_str($data['journal_id']);
			$this->db->where('journal_id', $data['journal_id'], FALSE);
		}
