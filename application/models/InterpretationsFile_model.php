...
class InterpretationsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'rule_file_';
		$this->from = 'interpretations_file';
		$this->id = 'rule_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['rule_id']) && $data['rule_id'] !=''){
			$data['rule_id'] = $this->db->escape_str($data['rule_id']);
			$this->db->where('rule_id', $data['rule_id'], FALSE);
		}
