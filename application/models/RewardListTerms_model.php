...
class RewardListTerms_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'reward_list_terms_';
		$this->from = 'reward_list_terms';
		$this->id = 'reward_list_terms_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
		$this->orderBy = 'sort';
		$this->orderType = 'asc';
	...
	public function deleteDataList($data){
		...
		if( isset($data['reward_list_chapter_id']) && $data['reward_list_chapter_id'] !=''){
			$data['reward_list_chapter_id'] = $this->db->escape_str($data['reward_list_chapter_id']);
			$this->db->where('reward_list_chapter_id', $data['reward_list_chapter_id'], FALSE);
		}
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			...
			$this->db->where('reward_list_terms_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('reward_list_terms_created_date <=', $queryData['endDate'], FALSE);
		}

		if( isset($queryData['reward_list_chapter_id']) && $queryData['reward_list_chapter_id'] !=''){
			$queryData['reward_list_chapter_id'] = $this->db->escape($queryData['reward_list_chapter_id']);
			$this->db->where('reward_list_chapter_id', $queryData['reward_list_chapter_id'], FALSE);
		}
