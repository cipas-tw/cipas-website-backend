...
class Albums_model extends MY_BackendModel {
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['photo_type_id']) && $queryData['photo_type_id'] !=''){
			$queryData['photo_type_id'] = $this->db->escape($queryData['photo_type_id']);
			$this->db->where('photo_type_id', $queryData['photo_type_id'], FALSE);
		}

		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			$queryData['startDate'] = $this->db->escape($queryData['startDate'].' 00:00:00');
			$this->db->where('photo_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('photo_created_date <=', $queryData['endDate'], FALSE);
		}
