...
class RewardListChapter_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'reward_list_chapter_';
		$this->from = 'reward_list_chapter';
		$this->id = 'reward_list_chapter_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function getList($queryData, $limit){
		...
		if( isset($queryData['startDate']) && $queryData['startDate'] !=''){
			...
			$this->db->where('reward_list_chapter_created_date >=', $queryData['startDate'], FALSE);
		}

		if( isset($queryData['endDate']) && $queryData['endDate'] !=''){
			$queryData['endDate'] = $this->db->escape($queryData['endDate'].' 23:59:59');
			$this->db->where('reward_list_chapter_created_date <=', $queryData['endDate'], FALSE);
		}
