...
class RewardsFile_model extends MY_BackendModel {

	public function __construct(){
		parent::__construct();
		$this->abrv = 'reward_file_';
		$this->from = 'rewards_file';
		$this->id = 'reward_file_id';
		$this->listSelect = '*';
		$this->rowSelect = '*';
	...
	public function deleteDataList($data){
		...
		if( isset($data['reward_id']) && $data['reward_id'] !=''){
			$data['reward_id'] = $this->db->escape_str($data['reward_id']);
			$this->db->where('reward_id', $data['reward_id'], FALSE);
		}
