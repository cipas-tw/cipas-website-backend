												...
												<div class="row">
													<div class="col-md-4">
														<div class="form-group">
		                                                    <label for="journal_type_id" class="control-label">分類</label>
	                                                        <select class="form-control select2" id="journal_type_id" name="journal_type_id">
	                                                        <option value="">請選擇</option>
	                                                        <?php foreach($journalTypeList as $type){?>
	                                                            <option value="<?php echo $type['journal_type_id']?>" <?php echo isset($queryData['journal_type_id']) && $queryData['journal_type_id'] == $type['journal_type_id'] ? 'selected' : ''?>><?php echo $type['journal_type_name']?></option>
	                                                        <?php }?>
	                                                        </select>
		                                                </div>
													</div>
													<div class="col-md-4">
														<div class="form-group">
															<label class="control-label">發表日期</label>
															<div class="input-group input-large date-picker input-daterange" data-date-format="yyyy-mm-dd">
	                                                            <input type="text" class="form-control" name="startDate" value="<?php echo isset($queryData['startDate'])? $queryData['startDate'] : ''?>" placeholder="開始時間">
	                                                            <span class="input-group-addon"> ~ </span>
	                                                            <input type="text" class="form-control" name="endDate" value="<?php echo isset($queryData['endDate'])? $queryData['endDate'] : ''?>" placeholder="結束時間">
                                                            </div>
														</div>
													</div>
													<div class="col-md-12">
						...
						<div class="row" id="addBtn">
							<div class="col-md-12">
								<div class="form-group">
                                    ...
                                    <?php foreach($journalTypeList as $type){?>
                                    	<a href="/<?php echo $controllerName?>/typeEdit/<?php echo $type['journal_type_id']?>" class="btn blue"><i class="fa fa-plus"></i> <?php echo $type['journal_type_name']?>說明 </a>
                                    <?php }?>
								</div>
										...
										<table class="table table-striped table-bordered table-hover" id="sample_1">
											...
											<tbody>
												...
												<tr>
													...
													<td><?php echo $journalTypeList[$val[$abrv.'type_id']][$abrv.'type_name'];?></td>
													<td><?php echo $val[$abrv.'show_date'];?></td>
