...
<!--<![endif]-->
	...
	<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
										...
										<form id="formPost" action="" class="formPost form-horizontal form-bordered form-row-stripped" method="post">
											...
											<div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-9">
														<input type="text" placeholder="請填寫案由" class="form-control" id="<?php echo $abrv;?>title" name="<?php echo $abrv;?>title" aria-required="true" value="<?php echo isset($result[$abrv.'title']) ? stripslashes($result[$abrv.'title']) : '';?>" />
													</div>
												</div>
													...
													<div class="col-md-9">
														<table class="table table-striped table-bordered table-hover">
															<thead>
																...
																<tbody id="note_tbody">
																	<?php foreach($resultNote as $i=>$note){?>
																	<tr id="noteRow_<?php echo $i+1?>">
																		<input type="hidden" id="<?php echo $abrv;?>note_id[]" name="<?php echo $abrv;?>note_id[]" aria-required="true" value="<?php echo isset($file[$abrv.'note_id']) ? stripslashes($file[$abrv.'note_id']) : '';?>" />
																		<td>
																			<div class="form-group">
																				<label class="control-label col-md-3">日期</label>
																				<div class="col-md-9">
																					<div class="input-group input-medium date date-picker" data-date-format="yyyy-mm-dd">
							                                                            <input type="text" class="form-control" id="<?php echo $abrv;?>note_date[]" name="<?php echo $abrv;?>note_date[]" value="<?php echo isset($note[$abrv.'note_date']) ? $note[$abrv.'note_date'] : '';?>" readonly>
																			...
																			<div class="form-group">
						                                                        ...
						                                                        <div class="col-md-9">
																					<input type="text" placeholder="請填寫標題" class="form-control" id="<?php echo $abrv;?>note_title[]" name="<?php echo $abrv;?>note_title[]" aria-required="true" value="<?php echo isset($note[$abrv.'note_title']) ? stripslashes($note[$abrv.'note_title']) : '';?>" />
																				</div>
																			</div>
																			<div class="form-group">
																				<label class="control-label col-md-3">連結</label>
																				<div class="col-md-9">
																					<input type="text" placeholder="請填寫連結" class="form-control" id="<?php echo $abrv;?>note_hyperlinks[]" name="<?php echo $abrv;?>note_hyperlinks[]" aria-required="true" value="<?php echo isset($note[$abrv.'note_hyperlinks']) ? stripslashes($note[$abrv.'note_hyperlinks']) : '';?>" />
																				</div>
																			</div>
																			<div class="form-group">
																				...
																				<div class="col-md-9">
																					<?php if($i == 0){ ?>
																						<textarea class="ckeditor form-control" id="content" name="<?php echo $abrv;?>note_content[]" aria-required="true" rows="6"><?php echo isset($note[$abrv.'note_content']) ? stripslashes($note[$abrv.'note_content']) : '';?></textarea>
							                                                        	<div id="<?php echo $abrv;?>note_content[]"></div>
																					<?php }else{?>
																						<textarea class="form-control" id="<?php echo $abrv;?>note_content[<?=$i?>]" name="<?php echo $abrv;?>note_content[]" aria-required="true" rows="6"><?php echo isset($note[$abrv.'note_content']) ? stripslashes($note[$abrv.'note_content']) : '';?></textarea>
							                                                        	<div id="<?php echo $abrv;?>note_content[<?=$i?>]"></div>
																					<?php }?>
		...
		jQuery(document).ready(function() {
			<?php foreach($resultNote as $i=>$note){?>
				<?php if($i != 0){ ?>
					CKEDITOR.replace("hearing_note_content[<?=$i?>]");
				<?php }?>
			<?php }?>
		});
		</script>
