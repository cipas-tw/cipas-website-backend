...
class Articles extends MY_BackendController {
	public function __construct(){
		...
		$this->haveField = array($this->abrv.'title', $this->abrv.'type_id', $this->abrv.'show_date', $this->abrv.'content', $this->abrv.'status');

		$this->uploadFolder = $this->config->item('uploadPath');
		$this->tmpPath = $this->config->item('tmpPath');
		$this->uploadPath = $this->config->item('newsPath');
	...
	public function index(){
		...
		$newsTypeList = $this->models->getNewsTypeList();
		foreach($newsTypeList as $newsType){
			$data['newsTypeList'][$newsType[$this->abrv.'type_id']] = array(
				$this->abrv.'type_id' => $newsType[$this->abrv.'type_id'],
				$this->abrv.'type_name' => $newsType[$this->abrv.'type_name'],
			);
		}
	...
	public function create(){
		...
		if( $op === 'add' ){
			...
			unset($addData[$this->abrv.'file_title'],$addData['tmp_img_filename'], $addData['tmp_img_file_url']);
	...
	public function edit($id){
		...
		if( $op === 'upd' ){
			...
			if(isset($updateData['tmp_img_file_url']) && $updateData['tmp_img_file_url'] !=''){
				$updateData[$this->abrv.'orig_filename'] = $updateData['tmp_img_filename'];
				$updateData[$this->abrv.'filename'] = $updateData['tmp_img_file_url'];

				// 移動封面圖片
				foreach($this->cropSizeList as $size){

					$tmpPath = $this->uploadFolder.$this->tmpPath.$size['path'].$updateData[$this->abrv.'filename'];
					$filePath = $this->uploadFolder.$this->uploadPath.$size['path'].$updateData[$this->abrv.'filename'];
					$this->moveFile($tmpPath, $filePath);
				}
			}
			...
			unset($updateData[$this->abrv.'file_title'], $updateData[$this->abrv.'file_name'], $updateData[$this->abrv.'file_id'],$updateData['tmp_img_filename'], $updateData['tmp_img_file_url']);
		...
		$data['newsTypeList'] = $this->models->getNewsTypeList();
		$data['head']['title'] = $this->unit;
		$data['sidebar']['active'] = $this->controllerName;
		$data['template']['abrv'] = $this->abrv;
		$data['unit'] = $this->unit;
