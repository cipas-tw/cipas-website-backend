...
class Stories extends MY_BackendController {
	public function __construct(){
		parent::__construct();
		$this->unit = '史料故事';
		$this->abrv = 'history_story_';
		$this->load->model('Stories_model', 'models');
		$this->load->model('StoriesFile_model', 'fileModels');
		...
		$this->uploadPath = $this->config->item('historyStoryPath');
	...
	public function edit($id){
		...
		if( $op === 'upd' ){
			...
			$file_name = isset($dataList[$this->abrv.'file_name'])? $dataList[$this->abrv.'file_name'] : array();
			...
			if($status){
				...
				foreach($file_title as $i=>$title){
					...
					if(isset($tmp_file_url[$i]) && $tmp_file_url[$i]!=''){
						...
						if($file_name[$i] ==''){
							$setData[$this->abrv.'file_orig_name'] = null;
							$setData[$this->abrv.'file_name'] = null;
						}
					}
