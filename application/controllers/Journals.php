...
class Journals extends MY_BackendController {
	public function __construct(){
		parent::__construct();
		$this->unit = '期刊文獻';
		$this->abrv = 'journal_';
		$this->load->model('journals_model', 'models');
		$this->load->model('journalsFile_model', 'fileModels');
		...
		$this->uploadPath = $this->config->item('journalPath');
	...
	public function index(){
		...
		$journalTypeList = $this->models->getjournalTypeList();
		foreach($journalTypeList as $journalType){
			$data['journalTypeList'][$journalType[$this->abrv.'type_id']] = array(
				$this->abrv.'type_id' => $journalType[$this->abrv.'type_id'],
				$this->abrv.'type_name' => $journalType[$this->abrv.'type_name'],
			);
		}
	...
	public function typeEdit($journal_type_id){
		
		// haveField 為必填欄位，saveField 為所有要儲存的欄位
		$this->abrv = 'journal_type_';
		$this->saveField = array(
		...
		// 資料檢查
		$data = array();
		$data['result'] = $this->models->getJournalTypeData($journal_type_id);
		
		if($journal_type_id =='' || !isset($data['result'])){
			$alerts[$this->config->item('alertDanger')] = '查無此資料，請重新進入編輯頁面';
			$this->showAlerts($alerts);
			redirect('/'.$this->controllerName.'/');
		}
		...
		if( $op === 'upd' ){
			...
			// 儲存資料組成&檢查必填欄位
			$updateData = $this->postFieldChekck($saveField, $haveField, $dataList);
			$status = $this->models->updateTypeData($updateData);
		...
		$data['head']['title'] = $data['result']["journal_type_name"];
		$data['sidebar']['active'] = $this->controllerName;
		$data['unit'] = $data['result']["journal_type_name"];
		$data['abrv'] = $this->abrv;
		$data['httpGetParams'] = isset($_SERVER['HTTP_REFERER'])? $_SERVER['HTTP_REFERER'] : '';
